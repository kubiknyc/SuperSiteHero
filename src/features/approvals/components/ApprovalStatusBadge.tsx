...
const statusColorClasses: Record<ApprovalStatus, string> = {
  pending: 'bg-warning-light text-yellow-800 border-yellow-200',
  approved: 'bg-success-light text-green-800 border-green-200',
  approved_with_conditions: 'bg-info-light text-blue-800 border-blue-200',
  ...
  const badge = (
    ...
    >
      ...
      {status === 'approved_with_conditions' && conditions && showConditions && (
        <span title={conditions} className="text-primary">
          (!)
        </span>
      )}
    </span>
