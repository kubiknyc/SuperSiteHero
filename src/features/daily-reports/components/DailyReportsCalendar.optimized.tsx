/**
 * Daily Reports Calendar Component (Optimized)
 * Displays daily reports in a calendar view with status indicators
 *
 * Performance optimizations:
 * - Moved constant objects outside component
 * - Added useCallback for event handlers
 * - Existing useMemo for expensive calculations preserved
 */
...
// Status colors for indicators - moved outside component for stability
...
export function DailyReportsCalendar({
  ...
  // Group reports by date - already memoized ✓
  ...
  // Get dates that have reports for DayPicker modifiers - already memoized ✓
  const datesWithReports = useMemo(() => {
    return Object.keys(reportsByDate).map((dateStr) => new Date(dateStr))
  }, [reportsByDate])

  // Handle day click - Now memoized with useCallback
  ...
  }, [reportsByDate, navigate, projectId])

  // Get status indicator for a date - Wrapped in useCallback for stability
  const getStatusIndicator = useCallback((date: Date) => {
    const dateKey = format(date, 'yyyy-MM-dd')
    const reportsForDate = reportsByDate[dateKey]
  ...
  // Custom day content renderer - Wrapped in useCallback
  ...
  }, [reportsByDate, getStatusIndicator])

  // Calculate summary stats - already memoized ✓
  const stats = useMemo(() => {
